...
public class TransactionCapsuleTest extends BaseTest {
  ...
  @Test
  public void ecdsaSameKeyDifferentSignatureRejected() throws Exception {
    // Same key, two DIFFERENT valid signatures: the canonical low-s form and its
    // malleable high-s variant (s -> N - s, recovery parity flipped) recover the
    // same address. checkWeight must dedup by recovered address (VERSION_4_7_1) so
    // a single key cannot be counted twice toward a multi-sign threshold.
    ForkController forkController = ForkController.instance();
    forkController.init(chainBaseManager);
    long interval = chainBaseManager.getDynamicPropertiesStore().getMaintenanceTimeInterval();
    long hardForkTime =
        ((ForkBlockVersionEnum.VERSION_4_7_1.getHardForkTime() - 1) / interval + 1) * interval;
    chainBaseManager.getDynamicPropertiesStore().saveLatestBlockHeaderTimestamp(hardForkTime + 1);
    byte[] stats = new byte[27];
    Arrays.fill(stats, (byte) 1);
    chainBaseManager.getDynamicPropertiesStore()
        .statsByVersion(ForkBlockVersionEnum.VERSION_4_7_1.getValue(), stats);
    ECKey signer = new ECKey();
    ECKey other = new ECKey();
    List<ByteString> witnesses = new ArrayList<>();
    witnesses.add(ByteString.copyFrom(signer.getAddress()));
    chainBaseManager.getWitnessScheduleStore().saveActiveWitnesses(witnesses);
    Assert.assertTrue(forkController.pass(ForkBlockVersionEnum.VERSION_4_7_1));

    byte[] hash = Sha256Hash.hash(true, "dup-sig".getBytes());
    ECKey.ECDSASignature canonical = signer.sign(hash);
    ByteString sig1 = packEcdsaSig(canonical.v, canonical.r, canonical.s);
    BigInteger malleableS = ECKey.CURVE.getN().subtract(canonical.s);
    byte malleableV = (byte) (((canonical.v - 27) ^ 1) + 27);
    ByteString sig2 = packEcdsaSig(malleableV, canonical.r, malleableS);

    // Genuinely different signature bytes...
    Assert.assertNotEquals(sig1, sig2);
    // ...that recover the same signer address.
    byte[] addr1 = SignUtils.signatureToAddress(hash,
        TransactionCapsule.getBase64FromByteString(sig1), true);
    byte[] addr2 = SignUtils.signatureToAddress(hash,
        TransactionCapsule.getBase64FromByteString(sig2), true);
    Assert.assertArrayEquals(addr1, addr2);
    Assert.assertArrayEquals(signer.getAddress(), addr1);

    // Two keys so checkWeight's "more signatures than keys" guard does not fire first.
    Permission permission = Permission.newBuilder()
        ...
        .addKeys(Key.newBuilder()
            .setAddress(ByteString.copyFrom(other.getAddress())).setWeight(1).build())
        .build();
  ...
  private static ByteString packEcdsaSig(byte v, BigInteger r, BigInteger s) {
    byte[] raw = new byte[65];
    raw[0] = v;
    System.arraycopy(ByteUtil.bigIntegerToBytes(r, 32), 0, raw, 1, 32);
    System.arraycopy(ByteUtil.bigIntegerToBytes(s, 32), 0, raw, 33, 32);
